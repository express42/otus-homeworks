...
control 'terraform' do
  ...
  describe command('cd terraform && terraform init -backend=false && terraform validate') do
    its('stdout') { should match "Terraform has been successfully initialized!" }
    its('stderr') { should eq '' }
    its('exit_status') { should eq 0 }
  end

  describe command('cd terraform/prod && terraform init -backend=false && terraform validate') do
    its('stdout') { should match "Terraform has been successfully initialized!" }
    its('stderr') { should eq '' }
    its('exit_status') { should eq 0 }
  end

  describe command('cd terraform/stage && terraform init -backend=false && terraform validate') do
    its('stdout') { should match "Terraform has been successfully initialized!" }
    its('stderr') { should eq '' }
    its('exit_status') { should eq 0 }
  end
